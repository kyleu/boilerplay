...
class AuditRecordRowController @javax.inject.Inject() (
  ...
  def createForm = withSession("create.form", admin = true) { implicit request => implicit td =>
    ...
    Future.successful(Ok(views.html.admin.audit.auditRecordRowForm(
      request.identity, AuditRecordRow.empty(), "New Audit Record", cancel, call, isNew = true, debug = app.config.debug
    )))
  }
  ...
  def view(id: UUID, t: Option[String] = None) = withSession("view", admin = true) { implicit request => implicit td =>
    ...
    val notesF = app.coreServices.notes.getFor(request, "auditRecordRow", id)

    notesF.flatMap(notes => modelF.map {
      case Some(model) => renderChoice(t) {
        case MimeTypes.HTML => Ok(views.html.admin.audit.auditRecordRowView(request.identity, model, notes, app.config.debug))
  ...
  def editForm(id: UUID) = withSession("edit.form", admin = true) { implicit request => implicit td =>
    ...
    svc.getByPrimaryKey(request, id).map {
      case Some(model) => Ok(
        views.html.admin.audit.auditRecordRowForm(request.identity, model, s"Audit Record [$id]", cancel, call, debug = app.config.debug)
      )
      case None => NotFound(s"No AuditRecordRow found with id [$id].")
    }
  }
