...
class AuditRecordService @javax.inject.Inject() (override val tracing: TracingService) extends ModelServiceHelper[AuditRecord]("auditRecord") {
  def getByPrimaryKey(creds: Credentials, id: UUID)(implicit trace: TraceData) = {
    traceF("get.by.primary.key")(td => SystemDatabase.queryF(AuditRecordQueries.getByPrimaryKey(id))(td))
  }
  def getByPrimaryKeySeq(creds: Credentials, idSeq: Seq[UUID])(implicit trace: TraceData) = {
    traceF("get.by.primary.key.seq")(td => SystemDatabase.queryF(AuditRecordQueries.getByPrimaryKeySeq(idSeq))(td))
  }

  def getByModel(creds: Credentials, model: String, pk: Any*)(implicit trace: TraceData) = {
    SystemDatabase.queryF(AuditRecordQueries.GetByModel(model, pk))
  }

  override def countAll(creds: Credentials, filters: Seq[Filter] = Nil)(implicit trace: TraceData) = {
    traceF("get.all.count")(td => SystemDatabase.queryF(AuditRecordQueries.countAll(filters))(td))
  }
  override def getAll(
    creds: Credentials, filters: Seq[Filter], orderBys: Seq[OrderBy], limit: Option[Int], offset: Option[Int] = None
  )(implicit trace: TraceData) = {
    traceF("get.all")(td => SystemDatabase.queryF(AuditRecordQueries.getAll(filters, orderBys, limit, offset))(td))
  }

  // Search
  override def searchCount(creds: Credentials, q: String, filters: Seq[Filter])(implicit trace: TraceData) = {
    traceF("search.count")(td => SystemDatabase.queryF(AuditRecordQueries.searchCount(q, filters))(td))
  }
  override def search(
    creds: Credentials, q: String, filters: Seq[Filter], orderBys: Seq[OrderBy], limit: Option[Int], offset: Option[Int] = None
  )(implicit trace: TraceData) = {
    traceF("search")(td => SystemDatabase.queryF(AuditRecordQueries.search(q, filters, orderBys, limit, offset))(td))
  }

  def searchExact(
    creds: Credentials, q: String, orderBys: Seq[OrderBy] = Nil, limit: Option[Int] = None, offset: Option[Int] = None
  )(implicit trace: TraceData) = {
    traceF("search.exact")(td => SystemDatabase.queryF(AuditRecordQueries.searchExact(q, orderBys, limit, offset))(td))
  }

  def countByAuditId(creds: Credentials, auditId: UUID)(implicit trace: TraceData) = traceF("count.by.auditId") { td =>
    SystemDatabase.queryF(AuditRecordQueries.CountByAuditId(auditId))(td)
  }
  def getByAuditId(creds: Credentials, auditId: UUID, orderBys: Seq[OrderBy], limit: Option[Int], offset: Option[Int])(implicit trace: TraceData) = {
    traceF("get.by.auditId")(td => SystemDatabase.queryF(AuditRecordQueries.GetByAuditId(auditId, orderBys, limit, offset))(td))
  }
  def getByAuditIdSeq(creds: Credentials, auditIdSeq: Seq[UUID])(implicit trace: TraceData) = traceF("get.by.auditId.seq") { td =>
    SystemDatabase.queryF(AuditRecordQueries.GetByAuditIdSeq(auditIdSeq))(td)
  }

  // Mutations
  def insert(creds: Credentials, model: AuditRecord)(implicit trace: TraceData) = {
    traceF("insert")(td => SystemDatabase.executeF(AuditRecordQueries.insert(model))(td).flatMap {
  ...
  def insertBatch(creds: Credentials, models: Seq[AuditRecord])(implicit trace: TraceData) = {
    traceF("insertBatch")(td => SystemDatabase.executeF(AuditRecordQueries.insertBatch(models))(td))
  }
  def create(creds: Credentials, fields: Seq[DataField])(implicit trace: TraceData) = traceF("create") { td =>
    SystemDatabase.executeF(AuditRecordQueries.create(fields))(td)
  ...
  def remove(creds: Credentials, id: UUID)(implicit trace: TraceData) = {
    traceF("remove")(td => getByPrimaryKey(creds, id)(td).flatMap {
      case Some(current) => SystemDatabase.executeF(AuditRecordQueries.removeByPrimaryKey(id))(td).map(_ => current)
  ...
  def update(creds: Credentials, id: UUID, fields: Seq[DataField])(implicit trace: TraceData) = {
    traceF("update")(td => getByPrimaryKey(creds, id)(td).flatMap {
      ...
      case Some(_) => SystemDatabase.executeF(AuditRecordQueries.update(id, fields))(td).flatMap { _ =>
