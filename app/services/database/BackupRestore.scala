...
object BackupRestore {
  ...
  def backup() = {
    ...
    if (f.exists) {
      util.ise(s"File [$filename] already exists.")
    }

    val zipFilename = filename + ".gz"
    val zf = backupRoot / zipFilename
    if (zf.exists) {
      util.ise(s"File [$zipFilename] already exists.")
    }
    ...
    if (!f.exists) {
      util.ise(s"Backup to file [$filename] failed: [$dumpResult].")
    }

    val gzipResult = Seq("gzip", path).!!
    if (!zf.exists) {
      util.ise(s"Compression of file [$zipFilename] failed: [$gzipResult].")
    }
