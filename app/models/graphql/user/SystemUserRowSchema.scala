...
object SystemUserRowSchema extends GraphQLSchemaHelper("systemUserRow") {
  ...
  private[this] def getByPrimaryKeySeq(c: GraphQLContext, idSeq: Seq[UUID]) = {
    c.injector.getInstance(classOf[services.user.SystemUserRowService]).getByPrimaryKeySeq(c.creds, idSeq)(c.trace)
  }
  ...
  val queryFields = fields(
    unitField(name = "systemUserRow", desc = None, t = OptionType(systemUserRowType), f = (c, td) => {
      c.ctx.injector.getInstance(classOf[services.user.SystemUserRowService]).getByPrimaryKey(c.ctx.creds, c.arg(systemUserRowIdArg))(td)
    }, systemUserRowIdArg),
    unitField(name = "systemUserRowSeq", desc = None, t = ListType(systemUserRowType), f = (c, td) => {
      c.ctx.injector.getInstance(classOf[services.user.SystemUserRowService]).getByPrimaryKeySeq(c.ctx.creds, c.arg(systemUserRowIdSeqArg))(td)
    }, systemUserRowIdSeqArg),
    unitField(name = "systemUserRowSearch", desc = None, t = systemUserRowResultType, f = (c, td) => {
      runSearch(c.ctx.injector.getInstance(classOf[services.user.SystemUserRowService]), c, td).map(toResult)
    }, queryArg, reportFiltersArg, orderBysArg, limitArg, offsetArg),
    unitField(name = "systemUserRowByUsername", desc = None, t = OptionType(systemUserRowType), f = (c, td) => {
      c.ctx.injector.getInstance(classOf[services.user.SystemUserRowService]).getByUsername(c.ctx.creds, c.arg(systemUserRowUsernameArg))(td).map(_.headOption)
    }, systemUserRowUsernameArg),
    unitField(name = "systemUsersByUsernameSeq", desc = None, t = ListType(systemUserRowType), f = (c, td) => {
      c.ctx.injector.getInstance(classOf[services.user.SystemUserRowService]).getByUsernameSeq(c.ctx.creds, c.arg(systemUserRowUsernameSeqArg))(td)
    }, systemUserRowUsernameSeqArg),
    unitField(name = "systemUserRowByProvider", desc = None, t = OptionType(systemUserRowType), f = (c, td) => {
      c.ctx.injector.getInstance(classOf[services.user.SystemUserRowService]).getByProvider(c.ctx.creds, c.arg(systemUserRowProviderArg))(td).map(_.headOption)
    }, systemUserRowProviderArg),
    unitField(name = "systemUsersByProviderSeq", desc = None, t = ListType(systemUserRowType), f = (c, td) => {
      c.ctx.injector.getInstance(classOf[services.user.SystemUserRowService]).getByProviderSeq(c.ctx.creds, c.arg(systemUserRowProviderSeqArg))(td)
    }, systemUserRowProviderSeqArg),
    unitField(name = "systemUserRowByKey", desc = None, t = OptionType(systemUserRowType), f = (c, td) => {
      c.ctx.injector.getInstance(classOf[services.user.SystemUserRowService]).getByKey(c.ctx.creds, c.arg(systemUserRowKeyArg))(td).map(_.headOption)
    }, systemUserRowKeyArg),
    unitField(name = "systemUsersByKeySeq", desc = None, t = ListType(systemUserRowType), f = (c, td) => {
      c.ctx.injector.getInstance(classOf[services.user.SystemUserRowService]).getByKeySeq(c.ctx.creds, c.arg(systemUserRowKeySeqArg))(td)
  ...
  val systemUserRowMutationType = ObjectType(
    ...
    fields = fields(
      unitField(name = "create", desc = None, t = OptionType(systemUserRowType), f = (c, td) => {
        c.ctx.injector.getInstance(classOf[services.user.SystemUserRowService]).create(c.ctx.creds, c.arg(dataFieldsArg))(td)
      }, dataFieldsArg),
      unitField(name = "update", desc = None, t = OptionType(systemUserRowType), f = (c, td) => {
        c.ctx.injector.getInstance(classOf[services.user.SystemUserRowService]).update(c.ctx.creds, c.arg(systemUserRowIdArg), c.arg(dataFieldsArg))(td).map(_._1)
      }, systemUserRowIdArg, dataFieldsArg),
      unitField(name = "remove", desc = None, t = systemUserRowType, f = (c, td) => {
        c.ctx.injector.getInstance(classOf[services.user.SystemUserRowService]).remove(c.ctx.creds, c.arg(systemUserRowIdArg))(td)
      }, systemUserRowIdArg)
    )
  )
