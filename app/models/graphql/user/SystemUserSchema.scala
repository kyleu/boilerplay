...
object SystemUserSchema extends GraphQLSchemaHelper("systemUser") {
  ...
  val systemUserByRoleFetcher = Fetcher.rel[GraphQLContext, SystemUser, SystemUser, UUID](
    getByPrimaryKeySeq, (c, rels) => c.injector.getInstance(classOf[SystemUserService]).getByRoleSeq(c.creds, rels(systemUserByRoleRelation))(c.trace)
  )
  ...
  implicit lazy val systemUserType: ObjectType[GraphQLContext, SystemUser] = deriveObjectType(
    AddFields(
      ...
      Field(
        ...
        resolve = c => c.ctx.noteLookup(c.ctx.creds, "systemUser", c.value.id)(c.ctx.trace)
      )
    )
  )
  ...
  val queryFields = fields[GraphQLContext, Unit](
    unitField(name = "profile", desc = None, t = profileType, f = (c, _) => {
      Future.successful(UserProfile.fromUser(c.ctx.creds.asInstanceOf[UserCredentials].user))
    }),
