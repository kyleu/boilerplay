...
object SyncProgressRowResult {
  implicit val jsonEncoder: Encoder[SyncProgressRowResult] = (r: SyncProgressRowResult) => io.circe.Json.obj(
  ...
  implicit val jsonDecoder: Decoder[SyncProgressRowResult] = (c: io.circe.HCursor) => for {
    ...
    results <- c.downField("results").as[Seq[SyncProgressRow]]
    durationMs <- c.downField("durationMs").as[Int]
    occurred <- c.downField("occurred").as[LocalDateTime]
  } yield SyncProgressRowResult(filters, orderBys, totalCount, paging, results, durationMs, occurred)
