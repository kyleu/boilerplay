...
object ScheduledTaskRunRowDoobie extends DoobieQueries[ScheduledTaskRunRow]("scheduled_task_run") {
  ...
  def getByPrimaryKey(id: UUID) = (selectFragment ++ whereAnd(fr"id = $id")).query[Option[ScheduledTaskRunRow]].unique
  def getByPrimaryKeySeq(idSeq: NonEmptyList[UUID]) = (selectFragment ++ in(fr"id", idSeq)).query[ScheduledTaskRunRow].to[Seq]
}
