...
object AuditRowResult {
  implicit val jsonEncoder: Encoder[AuditRowResult] = (r: AuditRowResult) => io.circe.Json.obj(
  ...
  implicit val jsonDecoder: Decoder[AuditRowResult] = (c: io.circe.HCursor) => for {
    ...
    results <- c.downField("results").as[Seq[AuditRow]]
    durationMs <- c.downField("durationMs").as[Int]
    occurred <- c.downField("occurred").as[LocalDateTime]
  } yield AuditRowResult(filters, orderBys, totalCount, paging, results, durationMs, occurred)
