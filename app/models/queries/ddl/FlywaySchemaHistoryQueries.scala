...
object FlywaySchemaHistoryQueries extends BaseQueries[FlywaySchemaHistory]("flywaySchemaHistory", "flyway_schema_history") {
  ...
  def insert(model: FlywaySchemaHistory) = new Insert(model)
  def insertBatch(models: Seq[FlywaySchemaHistory]) = new InsertBatch(models)
  ...
  override def fromRow(row: Row) = FlywaySchemaHistory(
